...
//----------------------------------------------------------------------------
template <typename Real>
Vector2<Real>::Vector2 (const Real * pXY)
{
	mTuple[0] = pXY[0];
	mTuple[1] = pXY[1];
}
...
//----------------------------------------------------------------------------
template <typename Real>
inline Vector2<Real> Vector2<Real>::Normalized(const Real epsilon) const
{
	Real length = Length();
	if (length > epsilon)
	{
		Real invLength = ((Real)1) / length;
		return Vector2(mTuple[0] * invLength, mTuple[1] * invLength);
	}
	else
	{
