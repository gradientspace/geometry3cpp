...
{
public:
	virtual ~VertexSource() = default;
	virtual Vector3<Real> operator[](unsigned int i) const = 0;
};

template<typename Real>
class BasicVertexSource : public VertexSource<Real>
{
public:
	Vector3<Real> * pVertices;
	virtual ~BasicVertexSource() = default;
	virtual Vector3<Real> operator[]( unsigned int i ) const { return pVertices[i]; }
};


template <typename Real> WM5_MATHEMATICS_ITEM
void ComputeMassProperties (const VertexSource<Real> * pVertices, int numTriangles,
